<x-guest-layout>
    <h4 class="pb-4 font-bold text-3xl font-mali">Edit Penduduk</h4>
    <!-- Session Status -->
        ...
        <div class="mt-4">
            ...
            <select id="tahun_id" name="tahun_id" class="block mt-1 w-full form-select rounded-md" required>
                ...
                @foreach ($tahun as $item)
                    <option value="{{ $item->id }}" {{ $penduduk->tahun_id == $item->id ? 'selected' : '' }}>
                        {{ $item->tahun }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mt-4">
            ...
            <select id="kecamatan_id" name="kecamatan_id" class="block mt-1 w-full form-select rounded-md" required>
                ...
                @foreach ($kecamatan as $item)
                    <option value="{{ $item->id }}" {{ $penduduk->kecamatan_id == $item->id ? 'selected' : '' }}>
                        {{ $item->nama_kecamatan }}
                    </option>
                @endforeach
            </select>
